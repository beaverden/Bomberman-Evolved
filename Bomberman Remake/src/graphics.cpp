...
SDL_Surface* Graphics::loadImage(const std::string &filepath)
{
	...
	if (!loaded)
	{
		...
		if (!loaded)
		{
			if (Globals::DEBUG) printf("There has been an error loading from: %s: %s", filepath, SDL_GetError());
		}
		else
		{
...
TTF_Font* Graphics::loadFont(const std::string& fontName, const std::string& fontPath, const int fontSize)
{
	...
	if (!font)
	{
		...
		if (!font)
		{
			if (Globals::DEBUG) printf("Error loading %s font: %s", fontPath, SDL_GetError());
		}
		else
		{
			fonts[fontName] = font;
